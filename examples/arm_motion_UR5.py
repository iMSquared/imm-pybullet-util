...
"""This the copy of arm_motion.py with only change of robot (Kuka -> UR5)"""
...
# NOTE(ycho): to make `birrt` import work,
# set MOTION_LIB_PATH to an appropriate environment variable.
# should point to "https://github.com/caelan/motion-planners".
MOTION_LIB_PATH = os.environ.get(
    'MOTION_LIB_PATH',
    '/tmp/motion-planners')
...
def main():
    ...
    ee_pos_lo: Tuple3[float] = (-0.5, -0.5, 0)
    ee_pos_hi: Tuple3[float] = (+0.5, +0.5, +1)
    ...
    while True:
        ...
        for i, v in zip(joint_ids, q_src):
            bc.resetJointState(robot_id, i, v)
